...
// ── Example 1: x + 5 = 12 (رقم مُضاف) ──
const EXAMPLE_ADD = {
  equation: "x + 5 = 12",
  label: "مثال على الجمع",
  steps: [
    {
      text: "في هذه المعادلة، الرقم 5 مُضاف مع x. هذا يدل على أنها من النوع الأول: رقم مُضاف أو مطروح مع x.",
      display: "x + 5 = 12",
      audio: "/audio/add-sub-add-step1.mp3",
    },
    {
      text: "للتخلص من الرقم 5، ننقله إلى الطرف الآخر من المعادلة. وعند النقل تنعكس إشارته من جمع إلى طرح.",
      display: "x = 12 − 5",
      audio: "/audio/add-sub-add-step2.mp3",
    },
    {
      text: "نحسب الطرف الأيمن من المعادلة بعملية طرح بسيطة.",
      display: "x = 7",
      audio: "/audio/add-sub-add-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: 7 + 5 = 12 ✓",
      display: "الحل النهائي: x = 7",
      audio: "/audio/add-sub-add-step4.mp3",
      isFinal: true,
    },
  ],
};

// ── Example 2: x − 4 = 9 (رقم مطروح) ──
const EXAMPLE_SUB = {
  equation: "x − 4 = 9",
  label: "مثال على الطرح",
  steps: [
    {
      text: "في هذه المعادلة، الرقم 4 مطروح من x. هذا أيضاً من النوع الأول: رقم مُضاف أو مطروح مع x.",
      display: "x − 4 = 9",
      audio: "/audio/add-sub-sub-step1.mp3",
    },
    {
      text: "للتخلص من الرقم 4، ننقله إلى الطرف الآخر من المعادلة. وعند النقل تنعكس إشارته من طرح إلى جمع.",
      display: "x = 9 + 4",
      audio: "/audio/add-sub-sub-step2.mp3",
    },
    {
      text: "نحسب الطرف الأيمن من المعادلة بعملية جمع بسيطة.",
      display: "x = 13",
      audio: "/audio/add-sub-sub-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: 13 − 4 = 9 ✓",
      display: "الحل النهائي: x = 13",
      audio: "/audio/add-sub-sub-step4.mp3",
      isFinal: true,
    },
  ],
};
...
export default function Example_AddSub({ onNext }) {
  ...
  const sendToWhatsApp = () => {
    ...
    const msg =
      `📚 *سؤال من الطالب: ${name}*\n` +
      `📌 *المثال:* حل المعادلة ${example.equation} (رقم مُضاف أو مطروح مع x)\n\n` +
      `❓ *السؤال:*\n${question.trim()}`;
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* ── Header ── */}
        <div style={styles.eyebrow}>مثال محلول · رقم مُضاف أو مطروح مع x</div>
      ...
      <button onClick={() => { navigate("/questionsAddSub"); }} style={styles.exerciseBtn}>
        ابدأ التمرين ←
      </button>
      <style>{`
        @keyframes pulse {
