...
// ── Example 1: (2/3)x = 8 (كسر موجب) ──
const EXAMPLE_POS = {
  equation: "(2/3)x = 8",
  label: "مثال بكسر موجب",
  steps: [
    {
      text: "في هذه المعادلة، x مضروبة في الكسر 2/3. هذا يدل على أنها من النوع الرابع: كسر مضروب مع x.",
      display: "(2/3)x = 8",
      audio: "/audio/frac-pos-step1.mp3",
    },
    {
      text: "للتخلص من الكسر، نضرب طرفي المعادلة في مقلوبه. مقلوب الكسر 2/3 هو 3/2 (نعكس البسط والمقام).",
      display: "x = 8 × (3/2)",
      audio: "/audio/frac-pos-step2.mp3",
    },
    {
      text: "نضرب 8 في 3 ثم نقسم على 2، أو نبسّط مباشرة.",
      display: "x = 24/2",
      audio: "/audio/frac-pos-step3.mp3",
    },
    {
      text: "نبسّط الكسر النهائي لنحصل على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: (2/3) × 12 = 8 ✓",
      display: "الحل النهائي: x = 12",
      audio: "/audio/frac-pos-step4.mp3",
      isFinal: true,
    },
  ],
};

// ── Example 2: (−3/4)x = 9 (كسر سالب) ──
const EXAMPLE_NEG = {
  equation: "(−3/4)x = 9",
  label: "مثال بكسر سالب",
  steps: [
    {
      text: "في هذه المعادلة، x مضروبة في الكسر −3/4. هذا أيضاً من النوع الرابع: كسر مضروب مع x، لكن هذه المرة الكسر سالب.",
      display: "(−3/4)x = 9",
      audio: "/audio/frac-neg-step1.mp3",
    },
    {
      text: "نضرب طرفي المعادلة في مقلوب الكسر. مقلوب −3/4 هو −4/3 (تبقى الإشارة السالبة كما هي عند القلب).",
      display: "x = 9 × (−4/3)",
      audio: "/audio/frac-neg-step2.mp3",
    },
    {
      text: "نضرب 9 في −4 ثم نقسم على 3. موجب مضروباً في سالب يعطي ناتجاً سالباً.",
      display: "x = −36/3",
      audio: "/audio/frac-neg-step3.mp3",
    },
    {
      text: "نبسّط الكسر النهائي. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: (−3/4) × (−12) = 9 ✓",
      display: "الحل النهائي: x = −12",
      audio: "/audio/frac-neg-step4.mp3",
      isFinal: true,
    },
  ],
};
...
export default function Example_FracMult({ onNext }) {
  ...
  const sendToWhatsApp = () => {
    ...
    const msg =
      `📚 *سؤال من الطالب: ${name}*\n` +
      `📌 *المثال:* حل المعادلة ${example.equation} (كسر مضروب مع x)\n\n` +
      `❓ *السؤال:*\n${question.trim()}`;
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* ── Header ── */}
        <div style={styles.eyebrow}>مثال محلول · كسر مضروب مع x</div>
      ...
      <button onClick={() => { navigate("/questionsFractionMultiply"); }} style={styles.exerciseBtn}>
        ابدأ التمرين ←
      </button>
      <style>{`
        @keyframes pulse {
