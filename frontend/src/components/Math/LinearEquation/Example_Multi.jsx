...
// ── Example 1: 3x = 15 (معامل موجب) ──
const EXAMPLE_POS = {
  equation: "3x = 15",
  label: "مثال بمعامل موجب",
  steps: [
    {
      text: "في هذه المعادلة، الرقم 3 مضروب مباشرة في x (لا يوجد علامة + أو − بينهما). هذا يدل على أنها من النوع الثاني: رقم مضروب مع x.",
      display: "3x = 15",
      audio: "/audio/mult-pos-step1.mp3",
    },
    {
      text: "العملية المعكوسة للضرب هي القسمة. للتخلص من الرقم 3، نقسم طرفي المعادلة عليه.",
      display: "x = 15 ÷ 3",
      audio: "/audio/mult-pos-step2.mp3",
    },
    {
      text: "نحسب الطرف الأيمن من المعادلة بعملية قسمة بسيطة.",
      display: "x = 5",
      audio: "/audio/mult-pos-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: 3 × 5 = 15 ✓",
      display: "الحل النهائي: x = 5",
      audio: "/audio/mult-pos-step4.mp3",
      isFinal: true,
    },
  ],
};

// ── Example 2: −2x = 10 (معامل سالب) ──
const EXAMPLE_NEG = {
  equation: "−2x = 10",
  label: "مثال بمعامل سالب",
  steps: [
    {
      text: "في هذه المعادلة، الرقم −2 مضروب مباشرة في x. هذا أيضاً من النوع الثاني: رقم مضروب مع x، لكن هذه المرة المعامل سالب.",
      display: "−2x = 10",
      audio: "/audio/mult-neg-step1.mp3",
    },
    {
      text: "العملية المعكوسة للضرب هي القسمة. نقسم طرفي المعادلة على −2، وننتبه أن القسمة على رقم سالب تغيّر إشارة الناتج.",
      display: "x = 10 ÷ (−2)",
      audio: "/audio/mult-neg-step2.mp3",
    },
    {
      text: "نحسب الطرف الأيمن: موجب مقسوماً على سالب يعطي ناتجاً سالباً.",
      display: "x = −5",
      audio: "/audio/mult-neg-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: −2 × (−5) = 10 ✓",
      display: "الحل النهائي: x = −5",
      audio: "/audio/mult-neg-step4.mp3",
      isFinal: true,
    },
  ],
};

const EXAMPLES = [EXAMPLE_POS, EXAMPLE_NEG];

export default function Example_Mult({ onNext }) {
  ...
  const sendToWhatsApp = () => {
    ...
    const msg =
      `📚 *سؤال من الطالب: ${name}*\n` +
      `📌 *المثال:* حل المعادلة ${example.equation} (رقم مضروب مع x)\n\n` +
      `❓ *السؤال:*\n${question.trim()}`;
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* ── Header ── */}
        <div style={styles.eyebrow}>مثال محلول · رقم مضروب مع x</div>
      ...
      <button onClick={() => { navigate("/questionsMultiply"); }} style={styles.exerciseBtn}>
        ابدأ التمرين ←
      </button>
      <style>{`
        @keyframes pulse {
