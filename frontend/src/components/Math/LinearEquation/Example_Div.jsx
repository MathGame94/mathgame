...
// ── Example 1: x/4 = 7 (مقام موجب) ──
const EXAMPLE_POS = {
  equation: "x/4 = 7",
  label: "مثال بمقام موجب",
  steps: [
    {
      text: "في هذه المعادلة، x مقسومة على 4. هذا يدل على أنها من النوع الثالث: x مقسوم على رقم.",
      display: "x/4 = 7",
      audio: "/audio/div-pos-step1.mp3",
    },
    {
      text: "العملية المعكوسة للقسمة هي الضرب. للتخلص من القسمة على 4، نضرب طرفي المعادلة في 4.",
      display: "x = 7 × 4",
      audio: "/audio/div-pos-step2.mp3",
    },
    {
      text: "نحسب الطرف الأيمن من المعادلة بعملية ضرب بسيطة.",
      display: "x = 28",
      audio: "/audio/div-pos-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: 28 ÷ 4 = 7 ✓",
      display: "الحل النهائي: x = 28",
      audio: "/audio/div-pos-step4.mp3",
      isFinal: true,
    },
  ],
};

// ── Example 2: x/(−3) = 5 (مقام سالب) ──
const EXAMPLE_NEG = {
  equation: "x/(−3) = 5",
  label: "مثال بمقام سالب",
  steps: [
    {
      text: "في هذه المعادلة، x مقسومة على −3. هذا أيضاً من النوع الثالث: x مقسوم على رقم، لكن هذه المرة المقام سالب.",
      display: "x/(−3) = 5",
      audio: "/audio/div-neg-step1.mp3",
    },
    {
      text: "العملية المعكوسة للقسمة هي الضرب. نضرب طرفي المعادلة في −3، وننتبه أن الضرب في رقم سالب يغيّر إشارة الناتج.",
      display: "x = 5 × (−3)",
      audio: "/audio/div-neg-step2.mp3",
    },
    {
      text: "نحسب الطرف الأيمن: موجب مضروباً في سالب يعطي ناتجاً سالباً.",
      display: "x = −15",
      audio: "/audio/div-neg-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على قيمة x. يمكننا التحقق بتعويض القيمة في المعادلة الأصلية: −15 ÷ (−3) = 5 ✓",
      display: "الحل النهائي: x = −15",
      audio: "/audio/div-neg-step4.mp3",
      isFinal: true,
    },
  ],
};
...
export default function Example_Div({ onNext }) {
  ...
  const sendToWhatsApp = () => {
    ...
    const msg =
      `📚 *سؤال من الطالب: ${name}*\n` +
      `📌 *المثال:* حل المعادلة ${example.equation} (x مقسوم على رقم)\n\n` +
      `❓ *السؤال:*\n${question.trim()}`;
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* ── Header ── */}
        <div style={styles.eyebrow}>مثال محلول · x مقسوم على رقم</div>
      ...
      <button onClick={() => { navigate("/questionsDivide"); }} style={styles.exerciseBtn}>
        ابدأ التمرين ←
      </button>
      <style>{`
        @keyframes pulse {
