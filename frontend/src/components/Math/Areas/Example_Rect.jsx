...
// ── Example 1: مستطيل طوله 6 سم وعرضه 4 سم ──
const EXAMPLE_RECT = {
  equation: "مستطيل طوله 6 سم وعرضه 4 سم",
  label: "مثال على المستطيل",
  steps: [
    {
      text: "لدينا مستطيل، وقانون مساحته هو ضرب الطول في العرض. الطول هنا 6 سم والعرض 4 سم.",
      display: "م = ط × ع",
      audio: "/audio/rect-step1.mp3",
    },
    {
      text: "نعوّض القيم المعطاة في القانون: الطول 6 والعرض 4.",
      display: "م = 6 × 4",
      audio: "/audio/rect-step2.mp3",
    },
    {
      text: "نضرب الرقمين معاً لنحصل على المساحة.",
      display: "م = 24",
      audio: "/audio/rect-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على مساحة المستطيل. لا ننسى أن نضيف الوحدة المربعة لأن المساحة تُقاس بوحدات مربعة.",
      display: "الحل النهائي: م = 24 سم²",
      audio: "/audio/rect-step4.mp3",
      isFinal: true,
    },
  ],
};

// ── Example 2: مربع طول ضلعه 9 سم ──
const EXAMPLE_SQUARE = {
  equation: "مربع طول ضلعه 9 سم",
  label: "مثال على المربع",
  steps: [
    {
      text: "لدينا مربع، وهو مستطيل خاص أضلاعه الأربعة متساوية. قانون مساحته هو الضلع مضروباً في نفسه.",
      display: "م = ض × ض = ض²",
      audio: "/audio/square-step1.mp3",
    },
    {
      text: "نعوّض طول الضلع المعطى، وهو 9 سم، في القانون.",
      display: "م = 9 × 9",
      audio: "/audio/square-step2.mp3",
    },
    {
      text: "نضرب الرقم في نفسه لنحصل على المساحة.",
      display: "م = 81",
      audio: "/audio/square-step3.mp3",
    },
    {
      text: "وبهذا حصلنا على مساحة المربع، مع إضافة الوحدة المربعة لأن المساحة تُقاس بوحدات مربعة.",
      display: "الحل النهائي: م = 81 سم²",
      audio: "/audio/square-step4.mp3",
      isFinal: true,
    },
  ],
};

const EXAMPLES = [EXAMPLE_RECT, EXAMPLE_SQUARE];

export default function Example_Rect({ onNext }) {
  ...
  const sendToWhatsApp = () => {
    ...
    const msg =
      `📚 *سؤال من الطالب: ${name}*\n` +
      `📌 *المثال:* حساب مساحة ${example.equation} (المربع والمستطيل)\n\n` +
      `❓ *السؤال:*\n${question.trim()}`;
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {/* ── Header ── */}
        <div style={styles.eyebrow}>مثال محلول · مساحة المربع والمستطيل</div>
      ...
      <button onClick={() => { navigate("/questionsRect"); }} style={styles.exerciseBtn}>
        ابدأ التمرين ←
      </button>
      <style>{`
        @keyframes pulse {
