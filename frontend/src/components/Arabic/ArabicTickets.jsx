...
const arabicTickets = [
  { id: 1, title: "النحو والصرف", done: true, description: "دراسة قواعد اللغة العربية وتركيب الكلمات والجمل.", topics: ["الجملة الاسمية والفعلية", "الإعراب والبناء", "أوزان الصرف", "المشتقات"] },
  { id: 2, title: "البلاغة", done: true, description: "علم البيان والمعاني والبديع في اللغة العربية.", topics: ["التشبيه", "الاستعارة", "الكناية", "المحسنات البديعية"] },
  { id: 3, title: "الأدب الجاهلي", done: true, description: "الشعر والنثر في العصر الجاهلي وخصائصه.", topics: ["المعلقات", "أغراض الشعر الجاهلي", "خصائص الأسلوب", "أشهر الشعراء"] },
  { id: 4, title: "الشعر العباسي", done: false, description: "تطور الشعر في العصر العباسي وأبرز شعرائه.", topics: ["خصائص الشعر العباسي", "أبو نواس", "المتنبي", "أبو تمام"] },
  { id: 5, title: "المقالة", done: false, description: "فن المقالة وأنواعها وعناصرها.", topics: ["المقالة الذاتية", "المقالة الموضوعية", "عناصر المقالة", "أساليب الكتابة"] },
  { id: 6, title: "القصة القصيرة", done: false, description: "عناصر القصة القصيرة وبناؤها الفني.", topics: ["الشخصيات", "الحدث", "الزمان والمكان", "الحبكة"] },
  { id: 7, title: "الأدب الحديث", done: false, description: "اتجاهات الأدب العربي الحديث وروافده.", topics: ["الرومانسية", "الواقعية", "الشعر الحر", "أبرز الأدباء"] },
  { id: 8, title: "النقد الأدبي", done: false, description: "مناهج النقد الأدبي وأدواته.", topics: ["النقد الانطباعي", "النقد الفني", "المنهج التاريخي", "المنهج النفسي"] },
];

const subjects = [
  { id: "math", name: "رياضيات", icon: "∑", color: "#4F46E5", active: false },
  { id: "arabic", name: "عربي", icon: "ض", color: "#059669", active: true },
  { id: "physics", name: "فيزياء", icon: "⚛", color: "#D97706", active: false },
  { id: "chemistry", name: "كيمياء", icon: "⚗", color: "#DC2626", active: false },
];

const COLOR = "#059669";
const done = arabicTickets.filter((t) => t.done).length;
const total = arabicTickets.length;
const pct = Math.round((done / total) * 100);

export default function ArabicTickets({ onStart }) {
  ...
  const selectedTicket = arabicTickets.find((t) => t.id === selected);
      ...
      <div style={{ ...styles.main, paddingBottom: isMobile ? "90px" : "32px" }}>
        <div style={styles.subjectHeader}>
          <div style={{ ...styles.iconCircle, background: COLOR }}>
            <span style={{ fontSize: 22, color: "#fff" }}>ض</span>
          </div>
          <div>
            <h2 style={{ ...styles.subjectName, color: COLOR }}>اللغة العربية</h2>
          ...
          {arabicTickets.map((t) => (
            <button key={t.id}
              ...
              style={{
                ...
                background: selected === t.id ? "#ECFDF5" : "#fff",
...
const styles = {
  ...
  sidebar: {
    width: 72, background: "#1E293B", display: "flex", flexDirection: "column",
    alignItems: "center", paddingTop: 48, gap: 6, flexShrink: 0,
  ...
  detailHeader: { padding: "24px 20px", display: "flex", alignItems: "center", gap: 14, color: "#fff" },
  detailBadge: {
